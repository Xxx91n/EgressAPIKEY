...
use resin_core::strategy::StrategyId;
use resin_core::strategy_engine::{AClassStrategy, PlatformStrategy, StrategyConfig};
...
async fn e2e_sub_then_platform_then_apply_in_wire_order() {
    ...
    // Whitebox side effects: one pipeline store entry (+1) and apply's GREEN
    // write-back (+1, ADR-0058 D3 — a green pass lands applied == generation
...
async fn e2e_twice_second_pass_zero_writes() {
    ...
    let m_subs = server
        ...
        .with_header("content-type", "application/json")
        .with_body(json!({"items": [{"id": "s1", "name": "e2e-sub", "node_count": 4}]}).to_string())
        .expect(6) // 3 subs GETs per pass (ensure+resolve+apply dangling) x 2 passes
        .create_async()
        .await;
    ...
    // duplicate platform/ref below). The generation DID advance 1 -> 3: each
    // green apply write-back bumps it (ADR-0058 D3, Terraform re-apply
    // semantics) — that is apply's own sanctioned store entry, not a pipeline
    // whitebox rewrite.
