...
impl MihomoController {
    ...
    pub async fn reload_config(&self, cfg: &MihomoConfig) -> Result<()> {
        let mut req = self
            .client
            .put(format!("{}/configs?force=true", self.api_base))
            .json(cfg);
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| anyhow!("mihomo reload request failed: {e}"))?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| anyhow!("mihomo version request failed: {e}"))?;
        if !resp.status().is_success() {
        ...
        let v: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| anyhow!("mihomo version parse: {e}"))?;
        v.get("version")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .ok_or_else(|| anyhow!("mihomo version payload missing 'version' field"))
    }
}
...
pub async fn compile_subscription(
    _url: &str,
    n_lanes: usize,
    mixed_port: u16,
) -> Result<MihomoConfig> {
...
mod tests {
    ...
    async fn compile_subscription_returns_skeleton() {
        let cfg = compile_subscription("https://example.invalid/sub.yaml", 10, 7897)
            .await
            .unwrap();
        assert_eq!(cfg.mixed_port, 7897);
        assert_eq!(cfg.pool_max_idle_per_host, 0);
