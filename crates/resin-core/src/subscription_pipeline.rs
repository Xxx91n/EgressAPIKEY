...
use crate::strategy_engine::{AClassStrategy, PlatformStrategy, StrategyId};
...
impl SubscriptionPipeline {
    ...
    fn retry_delay(attempts_so_far: u32) -> u64 {
        const BASE: u64 = 2;
        const CAP: u64 = 60;
        BASE.saturating_pow(attempts_so_far.min(16)).min(CAP)
    }
    ...
    pub async fn drain<S: StrategyConfigStore>(
        &self,
        client: &ResinClient,
...
pub async fn apply_strategy<S: StrategyConfigStore>(
    client: &ResinClient,
    svc: &StrategyService<S>,
) -> StepStatus {
...
pub async fn run_pipeline<S: StrategyConfigStore>(
    client: &ResinClient,
    svc: &StrategyService<S>,
    name: &str,
    url: &str,
...
mod tests {
    ...
    fn retry_delay_is_exponential_and_capped() {
        ...
        assert_eq!(SubscriptionPipeline::retry_delay(10), 60, "cap at 60s");
    }
    ...
    async fn mock_sub(server: &mut mockito::ServerGuard, name: &str, count: u64) -> mockito::Mock {
        server
            ...
            .expect(2) // step-1 presence read + step-2 resolve read
            .create_async()
            .await
    }
    ...
    async fn pipeline_success_all_five_steps_write() {
        ...
        // generation bumped exactly once by the pipeline's one store entry.
    ...
    async fn pipeline_idempotent_second_pass_zero_writes() {
        ...
        // whitebox generation did NOT bump: no store entry ran.
        let cfg = svc.get().unwrap();
        assert_eq!(cfg.generation, 1, "idempotent pass must not write the whitebox");
    ...
    async fn pipeline_partial_failure_apply_fails_after_platform_ok() {
        ...
        let mut server = mockito::Server::new_async().await;
        let m_subs = mock_sub(&mut server, "flaky", 5).await;
        let m_create_sub = server
            .mock("POST", "/api/v1/subscriptions")
            .match_header(BEARER.0, BEARER.1)
            .with_status(201)
        ...
        let m_nodes = server
            ...
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(json!({"items": [{"node_hash": "h1", "region": "US", "has_outbound": true, "failure_count": 0}]}).to_string())
            .expect(1)
            .create_async()
            .await;
        // The ONE failing write: the apply PATCH 500s.
        let m_patch = server
    ...
    async fn drain_backoff_retries_then_parks_after_max_attempts() {
        ...
        for attempt in 1..=MAX_ATTEMPTS {
            let reports = p.drain(&client, &svc, now + attempt * 120).await;
