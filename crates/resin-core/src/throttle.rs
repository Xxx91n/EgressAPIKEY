...
mod tests {
    ...
    fn clamp_applies_cap_then_floor_in_legacy_order() {
        ...
        assert_eq!(
            POLL.clamp_delay(Duration::from_secs(1)),
            Duration::from_secs(5)
        );
        assert_eq!(
            POLL.clamp_delay(Duration::from_secs(10)),
            Duration::from_secs(10)
        );
        assert_eq!(
            POLL.clamp_delay(Duration::from_secs(301)),
    ...
    fn clamp_without_cap_has_no_upper_bound() {
        assert_eq!(
            DELAY.clamp_delay(Duration::from_secs(86_400)),
            Duration::from_secs(86_400)
        );
        assert_eq!(
            DELAY.clamp_delay(Duration::from_secs(1)),
    ...
    fn adaptive_floor_and_growth_boundaries() {
        ...
        assert_eq!(adaptive_interval(POLL, 2.0, 1000), Duration::from_secs(14));
        // 2*ln1001 ~ 13.8 -> 14
    }
    ...
    fn backoff_doubles_with_consecutive_fails_until_exponent_cap() {
        ...
        assert_eq!(
            backoff_interval(POLL, base, 99, 5),
    ...
    fn backoff_caps_at_model_cap_and_floors_below_model_floor() {
        ...
        assert_eq!(
            backoff_interval(POLL, Duration::from_secs(200), 2, 5),
            Duration::from_secs(300)
        );
        // Floor binds when the product is small: 2s * 2^1 = 4s -> floored 5s.
        assert_eq!(
            backoff_interval(POLL, Duration::from_secs(2), 1, 5),
            Duration::from_secs(5)
        );
    }
