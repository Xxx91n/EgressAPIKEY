...
| DbPool single-lock (bb8) (critique #6) | lock-wait measured on a bench-selfhosted representative run -> reopen pool-impl adjudication (r2d2/deadpool). CANNOT FIRE while the selfhosted runner is unprovisioned (fallback 2026-10-20) | armed, gated on runner (r12-wave-d D-005) |
...
| ubuntu s1 waitForTitle flake | baseline: merged-state run 35755345313 both legs green; recurrence = the same s1 step fails again AFTER merged-state green (first failure never counts as recurrence) -> add retry | armed (r12-wave-d D-005) |
