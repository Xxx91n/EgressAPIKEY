...
| DbPool single-lock (bb8) (critique #6) | FOUR arms (r12-wave-e D-004, runner-gated dead arm rewritten): (i) dev-only lock-wait counter records a real-load wait >=1ms (threshold anchored to parking_lot eventual-fairness forcing line) -> reopen pool-impl adjudication (successor: deadpool-sqlite - r2d2 is 404-dead, recorded); (ii) synthetic concurrency probe (cargo test, 8 threads hammer list_ports) measures wait p99 >=1ms -> same; (iii) dev counter zero for two observation cycles AND probe p99 <1ms -> closed verdict discharge; (iv) hard date 2026-10-20: verdict MUST land regardless of runner provisioning (counter+probe substitute for representative run; instruments land with R12-E3) | **arm (ii) FIRED 2026-09-24** on first measurement - CI run 35959139983 (ubuntu verify, `concurrent_list_ports_lock_wait_probe_reports_p99`): acquisition-wait p99=9615us over 200 samples, 76/200 >=1ms, max=9879us -> pool-impl adjudication re-enters the ticket filter (successor deadpool-sqlite); probe kept as measuring instrument (reports p99 each run; assert = instrument sanity + 250ms hang-pathology bound, the fired 1ms arm not re-gated); arms (i)+(iv) remain armed (r12-wave-e D-004; supersedes r12-wave-d D-005 wording) |
...
- **read-path spawn_blocking asymmetry (D-004)**: OBSERVATION - replace_ports write path uses spawn_blocking (ADR-0011); read paths (list_ports/get_port via async IPC) hold the sync parking_lot lock directly. Tokio blesses short critical sections; exposure = read/write exemption inconsistency, not a lock-architecture error. Not a trigger line.
