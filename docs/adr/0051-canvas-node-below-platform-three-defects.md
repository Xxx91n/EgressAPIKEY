...
### atomcode 2026-08-25 deep-dive correction (dagre manual ranking audit)

Atomcode research #2 (source-code-level, 10 searches / 23 full reads): the manual-ranking feature (PR #271, merged 2025-06-17 as `@dagrejs/dagre@1.1.5`) is **thin** — the entire diff is +7/-1 lines across two files with **zero tests and zero docs** (the official wiki still does not list `none` as a valid ranker value). Production practice going into 2026 (Unsloth studio): teams **still** use phantom/anchor edges even on `@dagrejs/dagre` 1.x, precisely because manual rank has four known risks: (1) edges between two same-rank real nodes crash; (2) every node in the graph must carry an explicit `rank` attribute (miss one and `layers[undefined] is undefined` crashes); (3) `rank` goes through the `nodeNumAttrs` numeric channel (type-unsafe); (4) no tests in the merged commit.

**Recommendation**: upgrade from `dagre@0.8.5` to `@dagrejs/dagre@1.1.8` (1.x terminal, includes the empty-graph `bk.js` guard + perf fixes) — same call-site `dagre.layout()` works. But keep the anchor pattern (minlen:0 same-rank binding) — do not migrate to manual-rank pinning. If a future need arises for true rank pinning, spike it on a separate branch first (same-rank edges are the crash point).
