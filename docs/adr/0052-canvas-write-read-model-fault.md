...
- **Subscription edges are domain-un-deletable, not technically-locked**: auto-strategy edges (subscription/region/quality) carry `deletable: false` per CONTEXT.md ("only manual edges can be dragged/deleted"). React Flow v12 source-verified: `getElementsToRemove()` filters `deletable: false` upstream — such edges never reach `onEdgesDelete` or `deleteElements` as a silent no-op. The `isSub` branch inside `onEdgesDelete` is therefore dead code today, kept only as documentation of intent-removal semantics if the domain rule ever changes. This is a domain boundary (auto edges are not user-deletable from canvas), not a missing code path.
