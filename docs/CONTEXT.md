...
- **subscriptionRefresh** - the shell-initiated local subscription content refresh. The shell re-fetches the remote Clash YAML via `fetch_clash_subscription` (clash-family UA rotation — providers that 403 the default UA return 200 to a clash client UA), runs `clash_yaml_to_proxies_block` to convert flow-style inline mappings to block-style and strip proxy-groups/rules/dns, then PATCHes `/api/v1/subscriptions/{id}` with the new `content` + `source_type:"local"`. Necessary because Resin own `/actions/refresh` on a local source is a no-op re-parse of in-memory content. Triggered from NodesView per-group refresh button. See ADR-0044 S2.
