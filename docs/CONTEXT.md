...
- **defaultCollapsed** - NodesView subscription groups collapse by default after the first refresh (the `collapsed` Set is seeded with every subscription name). Users manually expand a group to see leaf nodes. Mirrors the industry mental model where metacubexd applies `hideUnAvailableProxies` and clash-verge #979 opts users in to batch testing rather than auto-expanding all visible. See ADR-0044 S1.
- **subscriptionRefresh** - the Resin-native subscription refresh. The shell calls `POST /api/v1/subscriptions/{id}/actions/refresh`; Resin's Scheduler re-pulls the remote URL via its own `clash.meta` UA fetcher (cmd/resin/main.go `const downloadUserAgent`). The shell no longer re-fetches or converts the Clash YAML itself (P13 B4 `fetch_clash_subscription`+`clash_yaml_to_proxies_block`+PATCH chain deleted). Refresh on a local-source subscription is a no-op re-parse of in-memory content, so the paired `subscription_add` migrated to `source_type:"remote"` in the same commit. Triggered from NodesView per-group refresh button. See ADR-0045.
- **sourceTypeRemote** - the value `"remote"` for the `source_type` field on `POST /api/v1/subscriptions`. Causes the Scheduler to HTTP-fetch the URL on every refresh tick. Its counterpart `"local"` re-parses the in-memory `content` blob instead (no HTTP), making `/actions/refresh` a no-op for a local subscription. ADR-0045 made `remote` the only path both `subscription_add` and `subscription_refresh` emit, so refresh actually pulls new nodes upstream.
- **nodeProbe** - manual or batch per-node ping via the Resin native `POST /api/v1/nodes/{hash}/actions/probe-egress` or `/actions/probe-latency` endpoints. Returns exit IP + EWMA latency (egress) or sync latency (latency). Whitebox `settings.json#nodeProbe` knobs: `concurrency`=10 (clash-verge hard cap), `timeout_ms`=10000 (clash-verge `DEFAULT_DELAY_TIMEOUT`), `test_url`=`https://www.gstatic.com/generate_204` (Resin default), `batch_on_load`=false (clash-verge #979 polite default). Dispatched from per-card test buttons and per-group batch test button. See ADR-0044 S3+S4.
- **probeKind** - the enum `{"egress","latency"}` carried by the `node_probe` IPC. `egress` returns the full `{egress_ip, latency_ms_ewma}` snapshot (heavier; Resin caps egress test interval at 24h); `latency` is a quick sync ping returning `{latency_ms}`. Both go through the same `node_probe` command, dispatched by `kind`; the shell does not implement its own probe logic. See ADR-0044 S3.

## T21 — Node Pool Button Sync (ADR-0046)

- **nodePoolToolbar** - the single inline `flex items-center gap-2` row on each subscription-group title bar in NodesView. Contains chevron + name + node-count + health-rate + RefreshCw IconButton + Activity IconButton as siblings (clash-rev HeadState pattern). The outer `<button>` was swapped to `<div + role="button" + tabIndex={0} + onKeyDown>` to avoid the nested-`<button>` spec violation (T20-v2 flagged the bug; ADR-0046 S1). Title bar IS the only action surface; no separate action band below. See ADR-0046 S1.
- **nodeProbe** (addendum, ADR-0046 S2) - per-probe results now propagate via the `applyProbeResult(prev, hash, kind, res)` pure helper immediately (clash-rev `DelayManager.setListener` per-proxy listener model); in a batch run each row updates its `-` placeholder to `ms` the moment its own probe resolves, not on batch completion. Batch progress state lives in `batchProgress: Map<string,{done,total}>`; the batch spinner tooltip interpolates `{{done}}/{{total}}` from this Map. See ADR-0046 S2.
- **subscriptionRefresh** (addendum, ADR-0046 S3) - the frontend pair (`handleRefreshSub` + `ipcSubscriptionRefresh`) runs synchronously with the backend: the toast `refreshSent` fires immediately on click, `ipcSubscriptionRefresh` blocks until Resin `Scheduler.UpdateSubscription` returns (remote fetch done), `refresh()` re-reads the now-fresh Resin memory, and a node-count delta check picks `refreshDone` / `refreshNoChange`. No delayed polling. See ADR-0046 S3.
