...
- **sourceTypeRemote** - the value `"remote"` for the `source_type` field on `POST /api/v1/subscriptions`. Causes the Scheduler to HTTP-fetch the URL on every refresh tick. Its counterpart `"local"` re-parses the in-memory `content` blob instead (no HTTP), making `/actions/refresh` a no-op for a local subscription. ADR-0047 made `remote` the only path both `subscription_add` and `subscription_refresh` emit, so refresh actually pulls new nodes upstream.
