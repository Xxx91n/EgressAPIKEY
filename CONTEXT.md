...
caused the connection, in strategy:value form: "manual:N", "region:US", "quality>75", "subscription:subA". For manual aClass, N = total manual_nodes on the platform; the edge targets every subscription/region group containing >=1 selected node_hash. Multiple edges
...
are non-deletable; only manual edges can be dragged/deleted; deleting a platform->groupX manual edge removes the manual_nodes belonging to groupX (other groups keep their selections).
