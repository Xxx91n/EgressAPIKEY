...
platform to a subscription on the canvas (or in the platform strategy panel)
records the subscription itself as the chosen strategy; the set of regions
the subscription covers is a DERIVED PROJECTION computed by the strategy
engine at apply time -- the canvas never hand-writes a region list when the
user's choice was a subscription. Corollary: an edge drawn because of a
specific subscription is labeled "subscription:subA"; a platform with no
subscription bound draws unlabeled unconditional edges (see
...
= all allowed) carries NO label -- the edge itself states "unrestricted",
per the Kiali/Grafana/LangGraph convention that default flows stay unlabeled
and only exceptions get annotated. For manual aClass, N = total manual_nodes
on the platform; the edge targets every subscription/region group containing
>=1 selected node_hash. Multiple edges from one platform to different nodes
indicate multiple strategies coexist. Inspired by Kiali's edge labels for
Istio routing rules. Auto-strategy edges are non-deletable; only manual edges
can be dragged/deleted; deleting a platform->groupX manual edge removes the
manual_nodes belonging to groupX (other groups keep their selections).
_Avoid_: routing line, connection tag, policy edge, subscription:all (the
"all allowed" state renders as an unlabeled edge, not a label)### Trace ID
