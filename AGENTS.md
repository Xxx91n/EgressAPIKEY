...
- **Config (`settings.json`)**: `app_config_dir()` — written by `tauri-plugin-store`. Keys: `lang`, `theme`, and GUI-only preference keys (`view`, `processRoutes`, WebDAV credentials, IP-reputation credentials, `keyCandidates`, `nodeProbe`, topology viewport / subscription order). Only the Rust tray reads `lang` directly; no other Rust component consumes settings.json (see §7.6). Legacy pre-T3-A network keys are purged once at startup (arch/02).
...
- **Database (observed_keys SQLite, C1-1)**: `crates/resin-core/src/db.rs` `DbPool(Arc<parking_lot::Mutex<rusqlite::Connection>>)` is opened in `src-tauri/src/main.rs` `.setup()` at `app_config_dir()/egressapikey.db` (WAL mode). The axum interceptor upserts one row per proxied request: `route_id` (`ar-<16hex>`, the same string injected as `X-Resin-Account`), `api_key_mask` (first4..last4), `endpoint` (upstream host), `first_seen/last_seen` (unix secs), `request_count`. The `observed_keys` IPC command (`commands/mod.rs`) exposes the pool to the webview; the Topology canvas joins `observed_keys` against the live lease map so each platform chip shows `sk-A...1234 · api.openai.com` instead of the raw `ar-<16hex>` account id. Schema migration = hand-written `PRAGMA user_version` (v1: single `observed_keys` table, `CREATE TABLE IF NOT EXISTS`). The Resin sidecar keeps its own separate `state.db`/`cache.db` under the per-user Resin state dir; `egressapikey.db` is the shell-owned key-observation log only.
...
platform_list = src-tauri/src/commands/mod.rs
platform_list_full = src-tauri/src/commands/mod.rs
platform_snapshot = src-tauri/src/commands/mod.rs
account_add = src-tauri/src/commands/mod.rs
account_bind_ip = src-tauri/src/commands/mod.rs
process_route_add = src-tauri/src/commands/mod.rs
process_route_remove = src-tauri/src/commands/mod.rs
process_route_list = src-tauri/src/commands/mod.rs
subscription_add = src-tauri/src/commands/mod.rs
subscription_remove = src-tauri/src/commands/mod.rs
subscription_refresh = src-tauri/src/commands/mod.rs
subscription_list = src-tauri/src/commands/mod.rs
node_pool_snapshot = src-tauri/src/commands/mod.rs
platform_update = src-tauri/src/commands/mod.rs
node_list = src-tauri/src/commands/mod.rs
node_probe = src-tauri/src/commands/mod.rs
platform_create_with_fields = src-tauri/src/commands/mod.rs
platform_leases = src-tauri/src/commands/mod.rs
system_config_get = src-tauri/src/commands/mod.rs
system_config_patch = src-tauri/src/commands/mod.rs
close_all_connections = src-tauri/src/commands/mod.rs
reset_kernel = src-tauri/src/commands/mod.rs
strategy_verify = src-tauri/src/commands/mod.rs
tray_refresh_labels = src-tauri/src/commands/mod.rs
get_config_dir = src-tauri/src/commands/mod.rs
get_log_dir = src-tauri/src/commands/mod.rs
get_sidecar_logs = src-tauri/src/commands/mod.rs
get_sidecar_status = src-tauri/src/commands/mod.rs
backup_create = src-tauri/src/commands/mod.rs
backup_upload = src-tauri/src/commands/mod.rs
backup_list = src-tauri/src/commands/mod.rs
config_export = src-tauri/src/commands/mod.rs
config_import = src-tauri/src/commands/mod.rs
lease_map = src-tauri/src/commands/mod.rs
ip_reputation_snapshot = src-tauri/src/commands/mod.rs
port_list = src-tauri/src/commands/mod.rs
port_suggest = src-tauri/src/commands/mod.rs
port_upsert = src-tauri/src/commands/mod.rs
port_remove = src-tauri/src/commands/mod.rs
port_toggle = src-tauri/src/commands/mod.rs
port_bind_platform = src-tauri/src/commands/mod.rs
port_running = src-tauri/src/commands/mod.rs
port_reload = src-tauri/src/commands/mod.rs
whitebox_save_network = src-tauri/src/commands/mod.rs
whitebox_path = src-tauri/src/commands/mod.rs
whitebox_get = src-tauri/src/commands/mod.rs
whitebox_reload = src-tauri/src/commands/mod.rs
stream_sensor_snapshot = src-tauri/src/commands/mod.rs
port_auth_info = src-tauri/src/commands/mod.rs
request_log_tail = src-tauri/src/commands/mod.rs
check_firewall_status = src-tauri/src/commands/mod.rs
probe_exit_ip = src-tauri/src/commands/mod.rs
port_health_check = src-tauri/src/commands/mod.rs
...
get_log_level = src-tauri/src/commands/mod.rs
watch_port_health = src-tauri/src/commands/mod.rs
```
