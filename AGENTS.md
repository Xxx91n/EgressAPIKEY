...
- The 18 locales are MT-maintained broad coverage with no human owner (r12-wave-a D-005): locale add/remove rides four trigger lines - demand (>=3 non-bot issues for one non-en/zh locale), adoption (>=15% non-English share once telemetry exists), friction (>=2 tray.rs locale reworks/yr or i18n-check fail >=1/mo), quality falsification (RTL/glyph/overflow) - retirement is clean (catalog + gates + tray arms together).
...
- GitHub Actions matrix builds five artifact groups into `release/`:
  ...
  3. macos-gui - Tauri `.dmg` arm64; x86_64 opt-in via `include_intel_mac` (macos-15-intel, to 2027-08)
  4. gui-portable - one Tauri `--no-bundle` GUI executable per OS - the drop-and-run variant
...
- iOS GUI note: iPadOS cannot run a Tauri shell; the Apple desktop sibling is the macOS `.dmg` (CI + README). No iOS build.
...
# node scripts/ipc-manifest-check.cjs --write. Commands: 87
```
